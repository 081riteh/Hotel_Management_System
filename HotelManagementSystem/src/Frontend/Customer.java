...
public class Customer {
	...
	void changePassword() throws SQLException {
		...
		do {
		    ...
		    if(pass==confirmPass) {
		    	...
		    	allQuery.changePassData(DBHome.getCon(),roomNo,pass);
		    }
		    else{
	...
	void orderFood() throws SQLException {
		...
		Map<String, Float> Menu = allQuery.getMenu(DBHome.getCon());
...
//		updateCustomerAmountData(roomNo,amount);
	...
	void provideBill(Map<String, Float> Menu) throws SQLException {
		...
		allQuery.updateCustomerAmountData(DBHome.getCon(),roomNo,billAmount);
