...
public class Receptionist {
	static int checkRoomAvailability() throws SQLException, ParseException {
		...
		ResultSet room=allQuery.checkRoomAvailability(DBHome.getCon(),roomSize,roomString);
	...
	static int bookRoom(int roomId) throws SQLException, ParseException {		
		...
		allQuery.addCustomerData(DBHome.getCon(),name,password,gender,age,aadharNo,phoneNo,0,roomId);
		return 0;
	}
	...
	static int checkout() throws SQLException {
		...
		String customerData="",checkOutDate="";
		Scanner sc=new Scanner(System.in);
		...
		do {
			...
			ResultSet data=allQuery.fetchCustomerData(DBHome.getCon(),roomNo);
			System.out.println(data);
			
			//String checkInDate=data.getString("check_in_date");
			System.out.println( data.getDate("check_in_date"));
			//System.out.println(checkInDate);
			while(data.next()) {
				System.out.println("|\tUser ID\t|\tEmp Name\t\t|\tAge\t|\tAadhar Number\t|\tPhone Number\t|");
				System.out.println("|\t"+data.getInt("user_id")+"\t|\t"+data.getString("user_name")+"\t\t|\t"+data.getInt("age")+"\t|\t"+data.getString("aadhar_no")+"\t|\t"+data.getString("phone_no")+"\t|");
			}
			...
			if(rechoose==1) {
				allQuery.changeAvailabilityData(DBHome.getCon(), roomNo, 1);
				allQuery.deleteCustomerData(DBHome.getCon(),roomNo);
				return 1;
			}
		}while(rechoose==2);
